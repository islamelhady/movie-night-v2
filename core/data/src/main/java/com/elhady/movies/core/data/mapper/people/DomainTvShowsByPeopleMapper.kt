...
class DomainTvShowsByPeopleMapper @Inject constructor() : Mapper<TvShowsCastItem, TvShow> {

    override fun map(input: List<TvShowsCastItem>): List<TvShow> {
        return input.map(::map)
    }

    override fun map(input: TvShowsCastItem): TvShow {
        return TvShow(
            id = input.id ?: 0,
            name = input.name ?: "",
